    ...
    func basicSwiftExecutable() async throws {
        try await withTemporaryDirectory { (tmpDir: Path) in
            ...
            try await tester.checkBuild(runDestination: destination) { results in
                ...
                let executionResult = try await Process.getOutput(url: URL(fileURLWithPath: wasmKitPath.str), arguments: ["run", projectDir.join("build").join("Debug-wasi").join("tool.wasm").str])
    ...
    func hostToolAndWasmExecutable() async throws {
        try await withTemporaryDirectory { (tmpDir: Path) in
            ...
            try await tester.checkBuild(runDestination: destination, buildRequest: request) { results in
                ...
                #expect(try tester.fs.read(projectDir.join("build/Debug-wasi/generated.txt")).unsafeStringValue == "Generated by host tool\n")

                let settings = results.buildRequestContext.getCachedSettings(results.buildRequest.parameters)
                let wasmKitPath = try #require(try settings.executableSearchPaths.lookup(subject: .executable(basename: "wasmkit"), operatingSystem: ProcessInfo.processInfo.hostOperatingSystem()))
                let executionResult = try await Process.getOutput(url: URL(fileURLWithPath: wasmKitPath.str), arguments: ["run", projectDir.join("build").join("Debug-wasi").join("WasmApp.wasm").str])
                #expect(executionResult.exitStatus == .exit(0))
                #expect(String(decoding: executionResult.stdout, as: UTF8.self) == "Hello from WebAssembly!\n")
                #expect(String(decoding: executionResult.stderr, as: UTF8.self) == "")
            }
    ...
    func unitTestWithGeneratedEntryPoint(framework: TestingFramework) async throws {
        try await withTemporaryDirectory(removeTreeOnDeinit: false) { (tmpDir: Path) in
            ...
            try await tester.checkBuild(runDestination: destination, persistent: true) { results in
                ...
                let settings = results.buildRequestContext.getCachedSettings(results.buildRequest.parameters)
                let wasmKitPath = try #require(try settings.executableSearchPaths.lookup(subject: .executable(basename: "wasmkit"), operatingSystem: ProcessInfo.processInfo.hostOperatingSystem()))
                let executablePath = projectDir.join("build").join("Debug-wasi").join("UnitTestRunner.wasm").str
